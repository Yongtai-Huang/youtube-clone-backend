...
exports.searchUser = asyncHandler(async (req, res, next) => {
  ...
  users.forEach(async (user, index) => {
    ...
    user.setDataValue("isSelf", isSelf);  // hyt: ??
...
exports.getProfile = asyncHandler(async (req, res, next) => {
  ...
  // subscribersCount, isSelf, isSubscribed
  const subscribersCount = await Subscription.count({
  ...
  user.setDataValue("isSelf", isSelf);
  ...
  const videos = await Video.findAll({
    ...
    attributes: ["id", "thumbnail", "title", "createdAt", "duration"]
  });
...
exports.getLikedVideos = asyncHandler(async (req, res, next) => {
  const Query = VideoLike.findAll({
    where: { [Op.and]: [{ userId: req.user.id }, { like: 1 }] },
    order: [["createdAt", "ASC"]]
  });

  return getVideos(Query, req, res, next);
});

exports.getHistory = asyncHandler(async (req, res, next) => {
  const Query = View.findAll({
...
const getVideos = async (Query, req, res, next) => {
  const videoRelations = await Query;

  if (!videoRelations.length) {
    return res.status(200).json({ success: true, data: videoRelations });
  }
  ...
  const videos = await Video.findAll({
    ...
    attributes: ["id", "title", "description", "createdAt", "thumbnail", "url", "duration"],
    include: {
      model: User,
